TYPE=VIEW
query=select `bdclick5`.`bdingreso`.`id` AS `id`,`bdclick5`.`bdcompra`.`producto` AS `producto`,`bdclick5`.`bdcompra`.`cantidad` AS `cantidad`,`bdclick5`.`bdingreso`.`bodega` AS `bodega`,`bdclick5`.`bdingreso`.`anulada` AS `anulada` from (`bdclick5`.`bdingreso` left join `bdclick5`.`bdcompra` on((`bdclick5`.`bdcompra`.`ingreso` = `bdclick5`.`bdingreso`.`id`))) where ((`bdclick5`.`bdingreso`.`id` <> \'ING-0\') and (`bdclick5`.`bdingreso`.`anulada` = 0))
md5=7d21aac829a4e757543da479b8c779ed
updatable=0
algorithm=0
definer_user=click
