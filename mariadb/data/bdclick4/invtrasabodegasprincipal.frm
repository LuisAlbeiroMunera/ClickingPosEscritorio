TYPE=VIEW
query=(select `invdevolucionesbodegas`.`codigo` AS `codigo`,`invdevolucionesbodegas`.`entradas` AS `entradas`,`bdclick4`.`invtrasaladosbodegas`.`salidas` AS `salidas`,(`bdclick4`.`invtrasaladosbodegas`.`salidas` - `invdevolucionesbodegas`.`entradas`) AS `Expr1` from (`bdclick4`.`invdevolucionesbodegas` join `bdclick4`.`invtrasaladosbodegas` on((`invdevolucionesbodegas`.`codigo` = `bdclick4`.`invtrasaladosbodegas`.`codigo`))))
md5=7812896d6b190ae45bee1f55b23a620c
updatable=0
algorithm=0
definer_user=root
